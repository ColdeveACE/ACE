        ...
        public void Decay(TimeSpan elapsed)
        {
            ...
            if (!TimeToRot.HasValue)
            {
                ...
                if (this is Corpse && Level.HasValue)
                    log.Info($"[CORPSE] {Name} (0x{Guid.ToString()}).Decay: TimeToRot had no value, set to {TimeToRot}");
            ...
            if (corpse != null)
            {
                ...
                if (corpse.Inventory.Count == 0 && TimeToRot.Value > Corpse.EmptyDecayTime)
                {
                    ...
                    if (Level.HasValue && PropertyManager.GetBool("corpse_decay_tick_logging").Item)
                        log.Info($"[CORPSE] {corpse.Name} (0x{corpse.Guid.ToString()}).Decay({elapsed.ToString()}): InventoryLoaded = {corpse.InventoryLoaded} | Inventory.Count = {corpse.Inventory.Count} | previous TimeToRot: {previousTTR} | current TimeToRot: {TimeToRot}");
                    return;
                }
            }
            ...
            if (TimeToRot > 0)
            {
                ...
                if (this is Corpse && Level.HasValue && PropertyManager.GetBool("corpse_decay_tick_logging").Item)
                    log.Info($"[CORPSE] {corpse.Name} (0x{corpse.Guid.ToString()}).Decay({elapsed.ToString()}): previous TimeToRot: {previousTTR} | current TimeToRot: {TimeToRot}");
            }
            ...
            if (corpse != null && !corpse.IsMonster)
            {
                ...
                log.Info($"[CORPSE] {corpse.Name} (0x{corpse.Guid.ToString()}) at {corpse.Location.ToLOCString()} has decayed{((pukedItems == "") ? "" : $" and placed the following items on the landblock: {pukedItems}")}.");
            }
